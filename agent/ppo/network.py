...
from agent.networks import BoundedRanker, make_scalar_head
...
POLICY_HEAD_VERSION = 7
SUPPORTED_POLICY_HEAD_VERSIONS = (7,)
CURRENT_PRODUCTION_RELATIVE_FEATURE_NAMES: tuple[str, ...] = (
...
CURRENT_WORKER_RELATIVE_FEATURE_NAMES: tuple[str, ...] = (
    "stage_duration_norm",
...
PRODUCTION_RELATIVE_FEATURE_NAMES = CURRENT_PRODUCTION_RELATIVE_FEATURE_NAMES
WORKER_RELATIVE_FEATURE_NAMES = CURRENT_WORKER_RELATIVE_FEATURE_NAMES
WORKER_RELATIVE_DIRECTIONS: tuple[float, ...] = (-1.0,) * len(
    CURRENT_WORKER_RELATIVE_FEATURE_NAMES
)
DEFAULT_CONTEXT_GATE_INITIAL_LOGIT = -4.0
CURRENT_CANDIDATE_CONTEXT_MODE = "bounded_ranker_scale_v7"
CURRENT_RELATIVE_WEIGHT_PARAMETERIZATION = "independent_softplus_signed_v" + "6"
CURRENT_WORKER_RELATIVE_WEIGHT_SHARING = "independent_softplus_v" + "6"
V7_BOUNDED_CONTEXT_MODE = CURRENT_CANDIDATE_CONTEXT_MODE
...
def _validate_policy_head_config(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    worker_enabled = bool(
        config.get("worker_candidate_relative_features", True)
    )
    ...
    allowed_production = (CURRENT_PRODUCTION_RELATIVE_FEATURE_NAMES,)
    allowed_worker = (CURRENT_WORKER_RELATIVE_FEATURE_NAMES,)
    ...
    context_mode = str(
        config.get("candidate_context_mode", CURRENT_CANDIDATE_CONTEXT_MODE)
    )
    if context_mode != CURRENT_CANDIDATE_CONTEXT_MODE:
        raise ValueError(
            "only bounded_ranker_scale_v7 is supported by the latest-only tree"
        )
    parameterization = str(
        config.get(
            "relative_weight_parameterization",
            CURRENT_RELATIVE_WEIGHT_PARAMETERIZATION,
        )
    )
    if parameterization != CURRENT_RELATIVE_WEIGHT_PARAMETERIZATION:
        raise ValueError(
            "network.relative_weight_parameterization must be "
            f"{CURRENT_RELATIVE_WEIGHT_PARAMETERIZATION!r}"
        )
    worker_sharing = str(
        config.get(
            "worker_relative_weight_sharing",
            CURRENT_WORKER_RELATIVE_WEIGHT_SHARING,
        )
    )
    if worker_sharing != CURRENT_WORKER_RELATIVE_WEIGHT_SHARING:
        raise ValueError(
            "network.worker_relative_weight_sharing must be "
            f"{CURRENT_WORKER_RELATIVE_WEIGHT_SHARING!r}"
        )
    common_gate = float(
        config.get(
    ...
    return {
        ...
        "worker_common_context_enabled": bool(
            config.get("worker_common_context_enabled", False)
        ),
...
def normalize_network_config(config: Mapping[str, Any]) -> dict[str, Any]:
    """Validate the sole supported HGNN architecture configuration."""

    if not isinstance(config, Mapping):
        raise TypeError("network config must be a mapping")
    encoder_type = str(config.get("encoder_type", "hetero_gnn"))
    if encoder_type != "hetero_gnn":
        raise ValueError("only encoder_type='hetero_gnn' is supported")
    if "hidden_dim" not in config:
        raise ValueError("network.hidden_dim is required")
    hidden_dim = int(config["hidden_dim"])
    message_passing_layers = int(config.get("message_passing_layers", 2))
    dropout = float(config.get("dropout", 0.0))
    if hidden_dim < 1:
        raise ValueError("network.hidden_dim must be positive")
    if message_passing_layers < 1:
        raise ValueError("network.message_passing_layers must be positive")
    ...
    return {
        ...
        "production_action_edge_features": bool(
            config.get("production_action_edge_features", True)
        ),
        "worker_action_edge_features": bool(
            config.get("worker_action_edge_features", True)
        ),
...
def _comparable_spec_value(name: str, value: Any) -> Any:
    if name in {
        "production_relative_feature_names",
        "worker_relative_feature_names",
        "production_relative_initial_weights",
...
def assert_network_config_matches_spec(
    ...
    normalize_network_config(configured)
    ...
    required_identity = {
        ...
        "worker_common_context_enabled": False,
        "residual_scale_ratio": 2.0,
        "action_set_feature_names": (),
    }
    for field, expected in required_identity.items():
        configured_value = _comparable_spec_value(
            field, configured.get(field, expected)
        )
        saved_value = _comparable_spec_value(field, saved.get(field))
        if configured_value != expected or saved_value != expected:
            raise ValueError(
                f"checkpoint {field} is incompatible with the current E1 network"
            )
    for field in _CURRENT_SPEC_FIELDS:
        if field in required_identity:
            continue
        left = _comparable_spec_value(field, configured.get(field))
        right = _comparable_spec_value(field, saved.get(field))
        if left != right:
            raise ValueError(
                f"checkpoint {field} is incompatible with the current E1 network: "
...
def infer_checkpoint_network_spec(
    ...
    """Read an explicit current E1 network spec; legacy inference is unsupported."""

    raw_spec = checkpoint.get("network_spec")
    if not isinstance(raw_spec, Mapping):
        raise ValueError(
            "checkpoint does not contain a current E1 network_spec; use the "
            "pre-refactor archive for historical checkpoints"
        )
    spec = dict(raw_spec)
    spec.update(normalize_network_config(spec))
    return spec
...
class HeteroGraphActorCritic(nn.Module):
    ...
    def __init__(
        ...
        production_relative_feature_names: Sequence[str] = (
            CURRENT_PRODUCTION_RELATIVE_FEATURE_NAMES
        ),
        worker_relative_feature_names: Sequence[str] = (
            CURRENT_WORKER_RELATIVE_FEATURE_NAMES
        ),
        candidate_context_mode: str = CURRENT_CANDIDATE_CONTEXT_MODE,
        ...
        self.production_scorer = make_scalar_head(
            self.hidden_dim * 4,
            self.hidden_dim,
            self.dropout_probability,
        )
        self.worker_scorer = make_scalar_head(
            self.hidden_dim * 5,
            self.hidden_dim,
            self.dropout_probability,
        )
        if self.use_production_candidate_relative_features:
            self.production_relative_ranker = BoundedRanker(
                len(self.production_relative_feature_names)
            )
        ...
        if self.use_worker_candidate_relative_features:
            self.worker_relative_ranker = BoundedRanker(
                len(self.worker_relative_feature_names)
            )
            self._initialize_monotone_ranker(
    ...
    def _context_residual(
        ...
        if self.candidate_context_mode != CURRENT_CANDIDATE_CONTEXT_MODE:
            raise RuntimeError("the current bounded ranker is not active")
